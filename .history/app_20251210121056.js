...
Object.keys(PROVINCES).sort().forEach(name => {
  const opt = document.createElement('option');
  opt.value = name;
  opt.textContent = `${name} (Vùng ${PROVINCES[name]})`;
  provinceSelect.appendChild(opt);
});

// Auto-set region when province changes
provinceSelect.addEventListener('change', function() {
  if (this.value && PROVINCES[this.value]) {
    document.getElementById('region').value = PROVINCES[this.value];
  }
});
